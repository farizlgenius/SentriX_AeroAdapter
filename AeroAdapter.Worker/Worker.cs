...
public class Worker(Channel<SCPReplyMessage> queue, ILogger<Worker> logger, IServiceScopeFactory scopeFactory) : BackgroundService
{
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        while (!stoppingToken.IsCancellationRequested)
        {
            ...
            consumer.ReceivedAsync += async (sender, ea) =>
            {
                ...
                Console.WriteLine("Command sent to AERO device ✈️");

                await channel.BasicAckAsync(ea.DeliveryTag, false);
            };

            await channel.BasicConsumeAsync("aero-device-command-queue", false, consumer);
